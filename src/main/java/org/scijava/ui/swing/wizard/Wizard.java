 ...
 * implements a basic wizard dialog, where the programmer can insert one or more
 ...
 * CardLayout manager, the order of the panels is not linear. Each panel
...
public class Wizard extends WindowAdapter implements PropertyChangeListener {
	 ...
	 */
	public Wizard(final Dialog owner) {
	 ...
	 *          javax.swing.JDialog.
	 */
	public Wizard(final Frame owner) {
		wizardModel = new WizardModel();
		wizardDialog = new JDialog(owner);
		initComponents();
	}
	 ...
	 */
	public void registerWizardPanel(final Object id,
		final WizardPanelDescriptor panel)
	{
	 ...
	 */
	public void setCurrentPanel(final Object id) {
		...
		if (oldPanelDescriptor != null) {
			if (!oldPanelDescriptor.aboutToHidePanel(id)) return; // transition
																														// rejected
		}

		final Object oldId =
			oldPanelDescriptor != null ? oldPanelDescriptor
				.getPanelDescriptorIdentifier() : null;
		if (!wizardModel.getPanelDescriptor(id).aboutToDisplayPanel(oldId)) return; // transition
																																								// rejected
	...
	public void close(final int code) {
		...
		if (oldPanelDescriptor != null && code == Wizard.FINISH_RETURN_CODE) {
			if (!oldPanelDescriptor.aboutToHidePanel(WizardPanelDescriptor.FINISH)) return; // transition
																																											// rejected
		}
