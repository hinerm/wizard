...
package org.scijava.ui.swing.wizard;
 ...
 * http://java.sun.com/developer/technicalArticles/GUI/swing/wizard/ A base
 * descriptor class used to reference a Component panel for the Wizard, as well
 * as provide general rules as to how the panel should behave.
 */
public class WizardPanelDescriptor {
	 ...
	 * the last panel and the text of the 'Next' button should change to 'Finish'.
	 */
	...
	/**
	 * Constructor which accepts both the Object-based identifier and a reference
	 * to the Component class which makes up the panel.
	 *
	 ...
	 */
	public WizardPanelDescriptor(final Object id, final Component panel) {
		panelIdentifier = id;
		targetPanel = panel;
	}
	 ...
	 */
	public final void setPanelDescriptorIdentifier(final Object id) {
		panelIdentifier = id;
	}

	final void setWizard(final Wizard w) {
		wizard = w;
	}
	 ...
	 * that the user should traverse to when the Next button is pressed. Note that
	 * this method is only called when the button is actually pressed, so that the
	 * panel can change the next panel's identifier dynamically at runtime if
	 * necessary. Return null if the button should be disabled. Return
	 * FinishIdentfier if the button text should change to 'Finish' and the dialog
	 * should end.
	 *
	...
	/**
	 * Override this class to provide the Object-based identifier of the panel
	 * that the user should traverse to when the Back button is pressed. Note that
	 * this method is only called when the button is actually pressed, so that the
	 * panel can change the previous panel's identifier dynamically at runtime if
	 ...
	 * be hidden.
	 */
	public boolean aboutToHidePanel(final Object idOfNext) {
		return true;
	}
